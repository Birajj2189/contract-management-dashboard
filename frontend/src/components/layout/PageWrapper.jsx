...
const PageWrapper = ({ title, description, actions, children, className, backFallback }) => {
  const backTo =
    backFallback != null && backFallback !== false
      ? typeof backFallback === 'string'
        ? backFallback
        : '/dashboard'
      : null
  ...
  return (
    <div className={cn('flex min-w-0 flex-col gap-6 sm:gap-8', className)}>
      {(title != null && title !== '') || actions || backTo ? (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          <div className="min-w-0 flex-1 space-y-2">
            {backTo ? (
              <div className="flex justify-start">
                <PageBackButton fallbackTo={backTo} className="-ml-2" />
              </div>
            ) : null}
            {title != null && title !== '' ? (
              typeof title === 'string' ? (
